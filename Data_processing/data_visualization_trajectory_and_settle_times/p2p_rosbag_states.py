...
#path = 'data/p2p_px4_done/run2.csv'
path = 'data/p2p_nmpc_done/run1.csv'
...
# Remove rows before second time the setpoint is updated, set point is data[19:22]
if not px4_data:
    setpoint_updates = np.where(np.any(data[:, 19:22] != data[0, 19:22], axis=1))[0]
else:
    setpoint_updates = np.where(np.any(data[:, 16:19] != data[0, 16:19], axis=1))[0]
if len(setpoint_updates) > 1:
    data = data[setpoint_updates[1]:, :]
...
# Create an ideal trajetory. The ideal trajectory is a straight line from the initial position to the setpoint over a total time of 3 seconds and then be constant for the rest of the setpoint time. The trajectory should be calculated for each time the state changes
state_changes = np.where(np.diff(states) != 0)[0] + 1
state_changes = np.insert(state_changes, 0, 0)  # Add the first index as a state change
...
axs[0].plot(time, load_setpoint[:,0], label='X Setpoint', color='black', alpha=0.75, linewidth=2)
axs[0].vlines(time[settling_index_x], color='tab:red', linestyle='--', label='X Settling Time', ymin=-3, ymax=3, alpha=0.75)
axs[0].plot(time, load_pose[:,0], label='Load X Position', color='tab:blue', linewidth=2)
#axs[0].plot(time, ideal_trajectory[:,0], label='Ideal X Trajectory', color='red', alpha=1, linewidth=2)
...
for i in range(1, data.shape[0]):
    if i == 1:
        if states[i] == 0:
            try:
                axs[0].axvspan(data[i, 0], data[i + np.where(states[i:] == 1)[0][0], 0], color='gray', alpha=0.25)
            except IndexError:
                axs[0].axvspan(data[i, 0], data[-1, 0], color='gray', alpha=0.25)
        else:
            try:
                axs[0].axvspan(data[i, 0], data[i + np.where(states[i:] == 0)[0][0], 0], color='lightgray', alpha=0.25)
            except IndexError:
                axs[0].axvspan(data[i, 0], data[-1, 0], color='lightgray', alpha=0.25)
    if states[i] != states[i-1]:
        if states[i] == 0:
            try:
                axs[0].axvspan(data[i, 0], data[i + np.where(states[i:] == 1)[0][0], 0], color='gray', alpha=0.25, label='Setpoint 1' if not green_shaded else None)
                green_shaded = True
            except IndexError:
                axs[0].axvspan(data[i, 0], data[-1, 0], color='gray', alpha=0.25)
        else:
            try:
                axs[0].axvspan(data[i, 0], data[i + np.where(states[i:] == 0)[0][0], 0], color='lightgray', alpha=0.25, label='Setpoint 2' if not red_shaded else None)
                red_shaded = True
            except IndexError:
                axs[0].axvspan(data[i, 0], data[-1, 0], color='lightgray', alpha=0.25)

axs[0].set_ylabel('X Position (m)', fontsize=14)
axs[0].legend(fontsize=14, loc='right')
axs[0].set_xlim(0, time[-1])
axs[0].set_ylim(-3, 3)
axs[0].grid()
if not px4_data:
    axs[0].set_title('Load Position using NMPC Controller', fontsize=16)
else:
    axs[0].set_title('Load Position using PX4 Position Controller', fontsize=16)

axs[1].plot(time, load_setpoint[:,1], label='Y Setpoint', color='black', alpha=0.75, linewidth=2)
axs[1].vlines(time[settling_index_y], color='tab:red', linestyle='--', label='Y Settling Time', ymin=-3, ymax=3, alpha=0.75)
axs[1].plot(time, load_pose[:,1], label='Load Y Position', color='tab:orange', linewidth=2)
#axs[1].plot(time, ideal_trajectory[:,1], label='Ideal Y Trajectory', color='red', alpha=1, linewidth=2)
...
for i in range(1, data.shape[0]):
    if i == 1:
        if states[i] == 0:
            try:
                axs[1].axvspan(data[i, 0], data[i + np.where(states[i:] == 1)[0][0], 0], color='gray', alpha=0.25)
            except IndexError:
                axs[1].axvspan(data[i, 0], data[-1, 0], color='gray', alpha=0.25)
        else:
            try:
                axs[1].axvspan(data[i, 0], data[i + np.where(states[i:] == 0)[0][0], 0], color='lightgray', alpha=0.25)
            except IndexError:
                axs[1].axvspan(data[i, 0], data[-1, 0], color='lightgray', alpha=0.25)
    if states[i] != states[i-1]:
        if states[i] == 0:
            try:
                axs[1].axvspan(data[i, 0], data[i + np.where(states[i:] == 1)[0][0], 0], color='gray', alpha=0.25, label='Setpoint 1' if not green_shaded else None)
                green_shaded = True
            except IndexError:
                axs[1].axvspan(data[i, 0], data[-1, 0], color='gray', alpha=0.25)
        else:
            try:
                axs[1].axvspan(data[i, 0], data[i + np.where(states[i:] == 0)[0][0], 0], color='lightgray', alpha=0.25, label='Setpoint 2' if not red_shaded else None)
                red_shaded = True
            except IndexError:
                axs[1].axvspan(data[i, 0], data[-1, 0], color='lightgray', alpha=0.25)

axs[1].set_ylabel('Y Position (m)', fontsize=14)
...
plt.tight_layout()
if not px4_data:
    plt.savefig('figures/p2p_nmpc_full_xy.png', dpi=300)
else:
    plt.savefig('figures/p2p_px4_full_xy.png', dpi=300)
plt.show()

start_index = state_changes[8]
end_index = state_changes[9] if len(state_changes) > 9 else data.shape[0]
...
axs[0,0].axhline(load_setpoint[start_index, 0], color='r', linestyle='--', label=f'Setpoint X={load_setpoint[start_index, 0]:.2f}m')
axs[0,0].set_ylabel('X Position (m)', fontsize=14)
axs[0,0].legend(fontsize=14)
axs[0,0].grid()
axs[0,0].set_ylim(x_limits)  # Set y-limits to match the calculated limits
...
axs[1,0].axhline(load_setpoint[start_index, 1], color='r', linestyle='--', label=f'Setpoint Y={load_setpoint[start_index, 1]:.2f}m')
axs[1,0].set_ylabel('Y Position (m)', fontsize=14)
axs[1,0].legend(fontsize=14)
axs[1,0].grid()
axs[1,0].set_ylim(y_limits)  # Set y-limits to match the calculated limits
axs[2,0].plot(time[start_index:end_index], load_pose[start_index:end_index,2], label='Load Z Position', color='tab:green')
axs[2,0].axhline(load_setpoint[start_index, 2], color='r', linestyle='--', label=f'Setpoint Z={load_setpoint[start_index, 2]:.2f}m')
axs[2,0].set_ylabel('Z Position (m)', fontsize=14)
axs[2,0].set_xlabel('Time (s)', fontsize=14)
axs[2,0].legend(fontsize=14)
axs[2,0].grid()
axs[2,0].set_ylim(z_limits)  # Set y-limits to match the calculated limits
...
plt.tight_layout()
if not px4_data:
    plt.savefig('figures/p2p_nmpc_states.png', dpi=300)
else:
    plt.savefig('figures/p2p_px4_states.png', dpi=300)
plt.show()
...
plt.tight_layout()
if not px4_data:
    plt.savefig('figures/p2p_nmpc_control_inputs.png', dpi=300)
else:
    plt.savefig('figures/p2p_px4_control_inputs.png', dpi=300)
...
ax.plot(load_pose[start_index:end_index,0], load_pose[start_index:end_index,1], load_pose[start_index:end_index,2], label='Payload Trajectory')
ax.plot(drone_pose[start_index:end_index,0], drone_pose[start_index:end_index,1], drone_pose[start_index:end_index,2], label='Drone Trajectory')
# Plot a small 3d model of the drone at 2 second intervals

# Use the 8th setpoint change again
snapshot_times = np.divide(np.array([0, 50, 80, 120, 170, 350]), 50).astype(int) * 240 + start_index
...
if not px4_data:
    plt.suptitle('3D Trajectory of Drone and Payload using NMPC Controller', fontsize=16)
else:
    plt.suptitle('3D Trajectory of Drone and Payload using PX4 Controller', fontsize=16)

if not px4_data:
    plt.savefig('figures/p2p_nmpc_3d_trajectory_with_snapshots.png', dpi=300)
else:
    plt.savefig('figures/p2p_px4_3d_trajectory_with_snapshots.png', dpi=300)
plt.show()

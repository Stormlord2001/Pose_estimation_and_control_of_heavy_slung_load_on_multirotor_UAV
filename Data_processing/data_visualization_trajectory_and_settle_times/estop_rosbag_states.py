...
# Single axes plot instead of a 1x1 subplot
fig, ax = plt.subplots(figsize=(10, 7))
if not px4_data:
    ax.set_title('Load Position using NMPC Controller', fontsize=16)
else:
    ax.set_title('Load Position using PX4 Controller', fontsize=16)
ax.plot(time, load_setpoint[:,1], label='Y Setpoint', color='black', alpha=0.75, linewidth=2)
ax.vlines(time[settling_index_y], color='tab:red', linestyle='--', label='Y Settling Time', ymin=-4, ymax=4, alpha=0.75)
ax.plot(time, load_pose[:,1], label='Load Y Position', color='tab:orange', linewidth=2)
...
for i in range(1, data.shape[0]):
    ...
    if states[i] != states[i-1]:
        if states[i] == 0:
            ...
            try:
                label_val = 'NMPC Controller' if not px4_data else 'Emergency Stop'
                label_val = label_val if not red_shaded else None
                ax.axvspan(data[i, 0], data[i + np.where(states[i:] == 0)[0][0], 0], color='lightgray', alpha=0.25, label=label_val)
                red_shaded = True
            except IndexError:
                ax.axvspan(data[i, 0], data[-1, 0], color='lightgray', alpha=0.25)
...
if not px4_data:
    plt.savefig('figures/estop_nmpc_full_y.png', dpi=300, bbox_inches='tight')
else:
    plt.savefig('figures/estop_px4_full_y.png', dpi=300, bbox_inches='tight')
plt.show()
...
axs[2,0].legend(fontsize=14, loc='upper right')
axs[2,0].grid()
axs[2,0].set_ylim(z_limits)
...
axs[2,1].legend(fontsize=14, loc='upper right')
axs[2,1].grid()
axs[2,1].set_title('Angular Velocity of Payload Vector', fontsize=16)
plt.tight_layout()
if not px4_data:
    plt.savefig('figures/estop_nmpc_states.png', dpi=300)
else:
    plt.savefig('figures/estop_px4_states.png', dpi=300)
plt.show()
...
if not px4_data:
    ax.set_title('3D Trajectory of Drone and Payload using NMPC Controller', fontsize=16)
else:
    ax.set_title('3D Trajectory of Drone and Payload using PX4 Position Controller', fontsize=16)
...
if not px4_data:
    plt.savefig('figures/estop_nmpc_3d_trajectory_with_snapshots.png', dpi=300)
else:
    plt.savefig('figures/estop_px4_3d_trajectory_with_snapshots.png', dpi=300)
plt.show()

...
with h5py.File('data/sim_data.mat', 'r') as f:
    data = np.array(f['data'])

with h5py.File('data/sim_control_output.mat', 'r') as f:
...
x_range = np.max(load_pose[:,0]) - np.min(load_pose[:,0])
y_range = np.max(load_pose[:,1]) - np.min(load_pose[:,1])
z_range = np.max(load_pose[:,2]) - np.min(load_pose[:,2])
max_range = max(x_range, y_range, z_range)*1.1

# Calculate a center value for each axis
x_center = (np.max(load_pose[:,0]) + np.min(load_pose[:,0])) / 2
y_center = (np.max(load_pose[:,1]) + np.min(load_pose[:,1])) / 2
z_center = (np.max(load_pose[:,2]) + np.min(load_pose[:,2])) / 2
...
axs[0,0].set_title('Load Position', fontsize=14)
...
axs[0,1].set_ylabel('Load Velocity (m/s)', fontsize=12)
axs[0,1].legend()
axs[0,1].grid()
axs[0,1].set_title('Load Velocity', fontsize=14)
...
axs[1,1].set_ylabel('Normalized Vector Components', fontsize=12)
axs[1,1].legend()
axs[1,1].grid()
axs[1,1].set_title('Cable Vector Components', fontsize=14)
...
axs[2,1].legend()
axs[2,1].grid()
axs[2,1].set_title('Angular Velocity of Payload Vector', fontsize=14)
plt.tight_layout()
...
axs[0].legend()
axs[0].grid()
axs[0].set_title('Thrust Input', fontsize=14)
...
axs[1].set_title('Torque Input', fontsize=14)
axs[1].legend()
axs[1].grid()
plt.tight_layout()
